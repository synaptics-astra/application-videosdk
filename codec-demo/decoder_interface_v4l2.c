...
void *rx_feedLoop(void *data)
{
	...
	while(pDec->is_running)
	{
		...
		if(pDec->is_running == 0)
			break;
		FeedBufferToV4L2H264Decoder(pCfg);
	}
...
static int startV4L2DecGstPipeline(dec_config_t *pCfg)
{
	...
	if(pCfg->src == NW_H264 || pCfg->src == FILE_H264)
	{
		ret = SetupV4L2H264Dec(pCfg);
		if(ret < 0)
		{
			LOG_ERR("Decoder cannot be initialized\n");
			return -1;
		}
		LOG_DEF("Decoder initialized\n");
...
void startV4L2Decoder(sess_config_t *sess_cfg)
{
	LOG_DEF("%s: \n", __func__);

	dec_config_t *pCfg = &sess_cfg->sess.dec_cfg;

	pCfg->priv = malloc(sizeof(priv_v4l2_decoder_t));
	priv_v4l2_decoder_t *pDec = (priv_v4l2_decoder_t *)pCfg->priv;

	memset(pDec, 0, sizeof(priv_v4l2_decoder_t));
...
void stopV4L2Decoder(sess_config_t *sess_cfg)
{
	dec_config_t *pCfg = &sess_cfg->sess.dec_cfg;
	priv_v4l2_decoder_t *pDec = (priv_v4l2_decoder_t *)pCfg->priv;
	...
	StopV4L2H264Decoder(pCfg);
}

void waitForV4L2DecoderShutdown(sess_config_t *sess_cfg)
{
	LOG_DEF("%s: \n", __func__);
